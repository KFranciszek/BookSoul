...
// 1️⃣  Serwowanie statycznych plików zbudowanego frontu
...
const startServer = async () => {
  try {
    ...
    // 2️⃣  Fallback SPA – każde żądanie, które NIE zaczyna się od /api/
    //     zwraca index.html, aby React Router mógł obsłużyć ścieżki
    app.get('*', (req, res, next) => {
      if (req.path.startsWith('/api/')) return next();      // zostaw API w spokoju
